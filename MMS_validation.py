...
#name = "GCP_Measure_edit5_speed_30_60"
name = "RTSD_GCP_Measure_speed_30"
df1 = pd.read_csv( name + ".csv")
#filter speed สำหรับการตรวจสอบ
df1 = df1[df1['Speed'] == 30] 
#df1 = pd.read_csv("RTSD_GCP_Measure_speed_30.csv")
for spd,row in df1.groupby('Speed'):
    ...
    row2[["dE2","dN2","dh2"]] = row2.apply(my_double,axis = 1,result_type = 'expand')
    ...
    # ใช้ชุดคำส่ังด้านล่างเพื่อเอาผลลัพธ์ออกมาเป็น csv
    #row2.to_csv(name + "row2.csv",index=False)
    #dfe.to_csv(name + "dfe.csv",index=False)
import pdb; pdb.set_trace()