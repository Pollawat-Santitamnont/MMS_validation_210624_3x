...
def pntden(folder,row):
    ...
    xmin,ymin,xmax,ymax = geom.Point(CEN[0], CEN[1]).buffer(2.5).bounds #setตามความยาวแนวทแยงของGCP ให้เกินหน่อยเพราะจะได้เก็บ GCP ได้ครบ
    df = df[ (df.X>xmin)   & (df.X<xmax)   &  (df.Y>ymin)  & (df.Y<ymax)]
    AREA = (df.X.max() - df.X.min())*(df.Y.max()-df.Y.min())
    Point_density = int(len(df)/AREA)
    return Point_density
    
if 0: ### auto get point density ###
...
if 1: ### get old pickle and plot
    #dfGCP = pd.read_pickle("./dfGCP_speed60.pkl") 
...
if 0: ### source for mutiple las ###
    ...
    for i in range(2):
        if i == 4: # be float nan so cant find density
            ...
            las = laspy.read( folder + "/"+ dfGCP["Strip"].iloc[i]) 
